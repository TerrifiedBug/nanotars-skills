...
function getPositionalAfterCommand() {
...
Environment:
  CALDAV_ACCOUNTS  JSON array of CalDAV account configs
    ...
    const accounts = (0, client_1.loadAccounts)();
    ...
    if (command === 'calendars') {
        for (const account of filtered) {
            const calendars = await (0, client_1.listCalendars)(account);
        ...
        const allEvents = [];
        for (const account of filtered) {
            const events = await (0, client_1.getEvents)(account, from, to, calendarName);
        ...
        const ok = await (0, client_1.createEvent)(account, calendarName, title, start, end, {
            location,
            description,
            allDay,
            rrule,
        ...
        const account = filtered[0];
        const ok = await (0, client_1.deleteEvent)(account, calendarName, title);
        if (!ok)
            process.exit(1);
    }
    else {
