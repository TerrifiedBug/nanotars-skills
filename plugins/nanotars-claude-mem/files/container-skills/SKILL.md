...
**Important:** Always use `project=nanotars` in all API calls. The hook writes observations under this project for every group, so queries scoped to it surface the full history.

## What's Captured Automatically

Every tool use (Bash, Read, WebSearch, MCP calls, etc.) is automatically saved to the database with the tool name, input, and output. You don't need to manually save tool results — they're already searchable. Relevant prior context is also auto-injected into the system prompt at session start, so you start each turn with recent memory in view; only reach for explicit search when the auto-injected slice doesn't cover what you need.
...
curl -s -X POST "$CLAUDE_MEM_URL/api/memory/save" \
  ...
  -d '{"text": "User prefers flat white coffee", "project": "nanotars"}'
```
...
```bash
curl -s "$CLAUDE_MEM_URL/api/search?query=morning+routine+preferences&project=nanotars" | jq -r '.content[0].text // .'
```
...
```bash
curl -s "$CLAUDE_MEM_URL/api/timeline?anchor=42&project=nanotars"
```
